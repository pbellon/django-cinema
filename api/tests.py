...
from cinema.models import Spectator, Movie, Author
...
@pytest.mark.django_db
def test_favorite_a_movie(authenticated_api_client: APIClient):
    movie = baker.make(Movie, title="Add to favorites")
    resp = authenticated_api_client.post(
        "/api/favorites/movies/", data={"movie_id": movie.id}
    )

    assert resp.status_code in (200, 204)

    # checks properly added to favorites
    resp = authenticated_api_client.get("/api/favorites/movies/")

    # import ipdb

    # ipdb.set_trace()

    assert any(m["id"] == movie.id for m in resp.data["results"])


@pytest.mark.django_db
def test_favorite_an_author(authenticated_api_client: APIClient):
    author = baker.make(
        Author, first_name="Some", last_name="Author", username="Some_Author"
    )
    resp = authenticated_api_client.post(
        "/api/favorites/authors/", data={"author_id": author.id}
    )

    assert resp.status_code in (200, 204)

    # checks properly added to favorites
    resp = authenticated_api_client.get("/api/favorites/authors/")
    assert any(a["id"] == author.id for a in resp.data["results"])
